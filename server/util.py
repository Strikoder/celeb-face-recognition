...
# we almost do the same as in notebook to get the X values
def classify_image(image_base64_data, file_path=None):
    ...
    for img in imgs:
        ...
        class_index = __model.predict(final)[0]
        class_name = class_number_to_name(class_index)
        result.append({
            'class': f"{class_name}: {probabilities[class_index]}%",
            'class_probabilities': {class_number_to_name(i): probability for i, probability in enumerate(probabilities)}
        })
